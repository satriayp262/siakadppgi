                ...
                <td class="px-4 py-2 text-center">
                    <div class="flex flex-col items-center space-y-2">
                        <div class="flex space-x-2">
                            <livewire:admin.dosen.edit wire:key="edit-{{$dosen->id_dosen}}" :id_dosen="$dosen->id_dosen" wire:key="edit-{{ $dosen->id_dosen }}" />
                        </div>
                        <button class="inline-block px-3 py-1 text-white bg-red-500 rounded hover:bg-red-700"
                                wire:click="destroy({{ $dosen->id_dosen }})" wire:confirm="Are you sure?">Delete</button>
                    </div>
                </td>
            </tr>
            @endforeach
