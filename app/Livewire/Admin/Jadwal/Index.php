...
class Index extends Component
{
    ...
    {
        $kelasList = Kelas::all()->shuffle(); // Shuffle classes for randomness
        ...
        $classSessions = []; // Track remaining SKS for each class

        // Initialize remaining SKS for each class
        ...
        while ($classIndex < $totalKelas) {
            foreach ($days as $day) {
                foreach ($timeSlots as $slotIndex => $timeSlot) {
                    ...
                    // Check for conflicts in the same room and time slot
                    ...
                    if (!$conflict) {
                        ...
                        // Deduct the session from remaining SKS
                        $classSessions[$kelas->id_kelas] -= 1;

                        // Move to the next class if fully scheduled
        ...
        session()->flash('success', 'Jadwal perkuliahan berhasil digenerate!');
    }
